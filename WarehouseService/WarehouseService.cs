...
{
    const string ServiceName = "WarehouseService";
    const string ExchangeName = "poc2.order.events";
    const string QueueName = "poc2.warehouse.orders";
    const string DeadQueue = "poc2.warehouse.dead";
    const string Dlx = "poc2.warehouse.dlx";
    const string DeadRoutingKey = "warehouse.dead";
    const string InitialRoutingKey = "order.submitted";
    const string RetryRoutingKey = "order.retry.warehouse";
    const string TransientMode = "WarehouseTransient";
    const string PermanentMode = "WarehousePermanent";
    ...
    static async Task Main()
    {
        ...
        var factory = new ConnectionFactory
        {
            ...
            ClientProvidedName = "POC2-WarehouseService"
        };
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[{ServiceName}] {order.OrderId} shipment prepared successfully.");
    ...
    {
        PermanentMode => "Simulated permanent warehouse failure",
        TransientMode => "Simulated transient warehouse failure",
        _ => "Unknown failure"
    };
}

...
{
    const string ServiceName = "InventoryService";
    const string ExchangeName = "poc2.order.events";
    const string QueueName = "poc2.inventory.orders";
    const string DeadQueue = "poc2.inventory.dead";
    const string Dlx = "poc2.inventory.dlx";
    const string DeadRoutingKey = "inventory.dead";
    const string InitialRoutingKey = "order.submitted";
    const string RetryRoutingKey = "order.retry.inventory";
    const string TransientMode = "InventoryTransient";
    const string PermanentMode = "InventoryPermanent";
    ...
    static async Task Main()
    {
        ...
        var factory = new ConnectionFactory
        {
            ...
            ClientProvidedName = "POC2-InventoryService"
        };
        ...
        consumer.ReceivedAsync += async (_, ea) =>
        {
            ...
            try
            {
                ...
                Console.WriteLine($"[{ServiceName}] {order.OrderId} stock reserved successfully.");
    ...
    {
        PermanentMode => "Simulated permanent inventory failure",
        TransientMode => "Simulated transient inventory failure",
        _ => "Unknown failure"
    };
}
